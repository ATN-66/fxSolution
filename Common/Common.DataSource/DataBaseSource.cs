...
namespace Common.DataSource
{
    public abstract class DataBaseSource : DataSource, IDataBaseSource
    {
        ...
        private readonly string _dataBaseSourceDateTimeFormat;
        ...
        protected override async Task<IList<Quotation>> GetDataAsync(DateTime startDateTimeInclusive)
        {
        ...
        private async Task<int> InsertDataAsync(SqlConnection connection, string tableName, DataTable dataTable, int weekNumber)
        {
            ...
            catch (Exception exception)
            {
                ...
                {
                    LogException(exRollback, MethodBase.GetCurrentMethod()!.Name, "Error during transaction rollback.");
                    throw;
                }
