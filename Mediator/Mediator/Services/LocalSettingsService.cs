...
public class LocalSettingsService : ILocalSettingsService
{
    ...
    private async Task InitializeAsync()
    {
        if (!_isInitialized)
        {
            _settings = await Task.Run(() => _fileService.Read<IDictionary<string, object>>(_applicationDataFolder, _localsettingsFile)).ConfigureAwait(false);
            if (_settings is null) { _settings = new Dictionary<string, object>(); }
